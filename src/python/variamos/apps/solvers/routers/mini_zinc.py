...
class ValidateMultiple(BaseModel):
    mx_graph: str
...
async def validate_feature_model(
    *,
    ...
    if "is_void" in operations:
        is_void = Validation(name="is_void", solution=mz_fm_solver.is_void_model())
        validations.append(is_void)

    if "is_fake_product_line" in operations:
        is_fake_product_line = Validation(
            name="is_fake_product_line", solution=mz_fm_solver.is_fake_product_line()
        )
